(function(root){
  ...
  function allPeriods(data,e,now=Date.now()){
    return Object.fromEntries([126,252].map(h=>{const a=inspect(data,e,h,now),f=a.researchForecast;
  ...
  function horizonPanel(data,e,h){
    const rows=[126,252].map(k=>{const a=inspect(data,e,k),f=a.researchForecast||e.predictions?.[k];return `<div class="periodCell ${k===h?'selected':''}"><b>${k===126?'6개월':'1년'}</b><strong>${f?pct(f.base/f.anchor-1)+'%':'—'}</strong><span>${a.eligible?'AI 검증 기준 통과':a.researchForecast?'AI 연구 · 검증 미통과':'기존 추세 · AI 자료 없음'}</span></div>`});
    ...
    return `<section class="periodComparison" aria-label="기간별 전망과 AI 검증"><div class="periodGrid">${rows.join('')}</div><p>6개월과 1년은 서로 분리해 학습합니다. 중간 경로는 일별 예측이 아닙니다. 1개월 목표가는 표시하지 않고 단기 진입 판단은 별도 규칙으로 계산합니다.</p><details><summary>기간별 검증 · 방향 기준선</summary><p>모델 ${esc(data?.model||'자료 없음')} · 자료 ${esc(data?.generatedAt?.slice(0,10)||'—')}. 방향 일치는 과거 결과이며 상승 확률이 아닙니다.</p>${[126,252].map(k=>{const m=data?.validation?.[k]||{};return `<p>${k===126?'6개월':'1년'}: ${m.dates||0}개 시험 시점 · AI 오차 ${pct(m.mae)}%p / 가격 불변 ${pct(m.noChangeMae)}%p · 방향 일치 ${pct(m.directionAccuracy)}% / 항상 상승 ${pct(m.alwaysUpAccuracy)}% · 하락 포착 ${pct(m.downRecall)}% · 전체 기준 ${m.passed?'통과':'미통과·자료 확인'}</p>`}).join('')}${d?`<p>선택 기간에서 두 비교 방법보다 오차가 작았던 시점: ${pct(d.dateWinRate)}%. 가장 나빴던 시험 시점의 평균 오차: ${pct(d.worstDateMae)}%p.</p><div class="marketTable"><table><thead><tr><th>당시 패턴</th><th>시점 수</th><th>AI 오차</th><th>불변 / 추세 오차</th></tr></thead><tbody>${Object.entries(d.regimes||{}).map(([k,m])=>`<tr><td>${labels[k]||esc(k)}</td><td>${m.dates||0}</td><td>${pct(m.mae)}%p</td><td>${pct(m.noChangeMae)} / ${pct(m.trendMae)}%p</td></tr>`).join('')}</tbody></table></div><p>패턴별 표는 같은 외부표본을 나눈 진단입니다. 소수 시점·겹치는 그룹의 결과로 패턴의 유효성을 확정할 수 없습니다.</p>`:'<p>확장된 패턴별 진단 결과는 아직 준비되지 않았습니다.</p>'}</details></section>`;
  }
  ...
  function panel(data,e,h,comparison){
    ...
    return `<div class="learnedSummary"><div class="learnedTitle"><b>${x.eligible?'AI 연구 전망 · 검증 기준 통과':f?'AI 연구 전망 · 검증 미통과':'AI 연구 자료 없음'}</b><span>${esc(x.record?.model||data?.model||'모델 확인 필요')}</span></div><p>${esc(x.reason)}</p>${f?`<p>모델 기준 ${esc(x.asOf)} · ${money(f.anchor)} → ${h}거래일 뒤 연구 목표 ${money(f.base)}. 최신 종가와 달라도 원래 목표가를 바꾸지 않습니다.</p>`:'<p>학습 결과가 없거나 유효하지 않아 기존 추세 계산을 참고용으로 표시합니다.</p>'}<div class="validationGrid"><div>수익률 평균 오차 · MAE<strong>${pct(m.mae)}%p</strong></div><div>방향 일치 / 항상 상승<strong>${pct(m.directionAccuracy)}% / ${pct(m.alwaysUpAccuracy)}%</strong></div><div>실제 하락 포착<strong>${pct(m.downRecall)}%</strong></div><div>검증 표본<strong>${m.n||0}회 · ${m.dates||0}개 시점</strong></div></div><p>방향은 상승·중립(±2%)·하락으로 비교합니다. 모델 방향 일치가 ‘항상 상승’ 기준보다 높고 실제 하락도 일부 포착해야 검증을 통과합니다. 과거 성적은 이번 예측의 성공 확률이 아닙니다.${(m.dates||0)<6?' 검증 시점이 6개 미만이므로 성적이 불안정합니다.':''}</p><details><summary>이 예측을 믿을 근거는?</summary><p>학습 결과 생성 ${esc(data?.generatedAt||'—')}. 수집 종목의 가격·거래량·SPY 흐름을 학습한 모델이며 뉴스·실적은 학습하지 않습니다.</p><p>같은 표본의 수익률 평균 오차: AI ${pct(m.mae)}%p / 가격 불변 ${pct(m.noChangeMae)}%p / 기존 추세 ${pct(m.trendMae)}%p. %p는 실제와 예상 수익률의 차이로, 현물 페이지의 가격 MAPE(%)와 다른 척도입니다.</p><p>전체 검증 ${g.dates||0}개 시점 / ${g.n||0}개 종목·시점 조합. 종목들이 같이 움직이므로 모두 독립된 시험은 아닙니다. 범위는 과거 교정 오차의 10·90 분위수이며 미래 포함 확률을 보장하지 않습니다. 검증 미통과 연구값의 표시는 매수 조건을 통과시킨다는 뜻이 아닙니다.</p><p>1년 목표가가 현재와 비슷해도 그 사이의 움직임은 클 수 있습니다. AI 목표 방향과 실제 변동성은 서로 다른 값입니다. 배당·거래비용·체결을 반영한 실전 성과는 아직 검증하지 않았습니다.</p></details></div>`;
  }
