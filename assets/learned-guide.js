(function(root){
  ...
  function allPeriods(data,e,now=Date.now()){
    return Object.fromEntries([21,84,252].map(h=>[h,inspect(data,e,h,now).forecast||e.predictions?.[String(h)]]));
  }
  function horizonPanel(data,e,h){
    const rows=[21,84,252].map(k=>{const a=inspect(data,e,k),f=a.forecast||e.predictions?.[k];return `<div class="periodCell ${k===h?'selected':''}"><b>${k===21?'1개월':k===84?'4개월':'1년'}</b><strong>${f?pct(f.base/e.price-1)+'%':'—'}</strong><span>${a.eligible?'AI 시범 채택':'AI 보류 · 기존 계산'}</span></div>`});
  ...
  const api={inspect,panel,nextSteps,comparisonPanel,allPeriods,horizonPanel};if(typeof module!=='undefined')module.exports=api;else root.LearnedGuide=api;
