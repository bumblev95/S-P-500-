...
const disclosure=(title,body)=>'<details class="site-disclosure"><summary>'+title+'</summary>'+body+'</details>';
function chart(a){
...
function render(){
 ...
 let html=strip+'<div class="workspace">'+panel('',graph)+'<aside>'+panel('',decision+'<div id="tradePlanner"></div>')+'</aside></div>';
 ...
 html+='<div class="detailsGrid">'+disclosure('왜 이런 방향인가?',kv('20 / 50봉 지수이동평균',money(a.ind.ema20)+' / '+money(a.ind.ema50))+V.rsi(a.ind.rsi)+kv('MACD 히스토그램',num(a.ind.macd))+kv('봉 기준 거래량가중평균',money(a.ind.vwap))+kv('ATR · 평균 봉 변동폭',money(a.ind.atr))+V.bars([{label:'20봉 방향성 효율',value:a.ind.efficiency*100,text:num(a.ind.efficiency)}])+'<p class="bodyNote">방향성 효율이 0에 가까우면 등락만 반복하는 횡보에 가깝습니다.</p>')+disclosure('다른 봉에서는?',rows+'<p class="bodyNote">5분봉은 15분봉, 15분봉은 1시간봉과 같은 방향이어야 진입 조건을 통과합니다. 일봉은 독립적으로 평가합니다.</p>')+panel('거래 비용과 유동성',kv('시간당 펀딩비',pct(q.funding))+kv('펀딩 방향',N(q.funding)?q.funding>0?'롱 지급 → 숏 수취':q.funding<0?'숏 지급 → 롱 수취':'현재 0':'—')+kv('24시간 선물 거래대금','$'+compact(q.volume24h))+kv('미결제약정','$'+compact(q.oiUSD))+kv('최우선 호가 간격',pct(snapshot.book?.spread))+kv('마크 / 오라클 괴리',pct(q.premium))+'<p class="bodyNote">손익비에는 편도 수수료 0.045%와 편도 슬리피지 가정 0.02%, 선택 주기 6봉 동안의 현재 지급 펀딩비를 반영합니다. 전망 표시 길이는 이 비용 가정이나 신호 유효기간을 바꾸지 않습니다. 실제 비용은 달라질 수 있습니다.</p>')+'</div>';
 ...
 el('app').innerHTML=html;window.DecisionSupport?.load().then(()=>window.DecisionSupport.mountPerp(symbol,snapshot,a));el('levelToggle').addEventListener('click',()=>{showLevels=!showLevels;render()});document.querySelectorAll('[data-ahead]').forEach(b=>b.addEventListener('click',()=>{const n=Number(b.dataset.ahead);if(E.HORIZONS[tf].includes(n)){aheadByTf[tf]=n;render()}}));
}
