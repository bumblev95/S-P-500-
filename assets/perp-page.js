...
function render(){
 ...
 if(window.SimulationSignals){const experiment=window.SimulationSignals.crypto(snapshot.frames['15m']||[],snapshot.frames['1h']||[]);graph+=panel('$10,000 모의운용 실험 신호 · 15분봉','<p><strong>'+(experiment.side==='long'?'롱 진입 대기':experiment.side==='short'?'숏 진입 대기':'관망')+'</strong> · '+esc(experiment.reason)+'</p><p class="bodyNote">세 가지 패턴 실험의 공통 계산입니다. 계좌는 서버에 기록한 신호 이후 다음 가용 시가로 가상 진입하며, 자동 갱신 지연·가격·자금 조건에 따라 체결하지 않을 수 있습니다. 기존 종합 조건 점수와 별도 실험입니다.</p><a href="simulation.html?asset=crypto&mode=forward">가상 계좌 · 매매 기록 보기 →</a>');}
