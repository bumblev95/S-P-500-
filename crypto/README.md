...
Stops sit 0.25 ATR beyond support/resistance, with risk <=5 ATR. Targets are observed confirmed pivots; after a breakout with no observed target, a measured range projection is explicitly labelled as an assumption. Targets are never moved to manufacture a passing reward/risk. The next-six-bar dotted chart extension is an untrained trend assumption. Scores are not probabilities; long and short scores need not sum to 100.
