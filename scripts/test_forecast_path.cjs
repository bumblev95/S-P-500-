...
for(const e of Object.values(data.stocks))for(const h of [126,252]){
...
for(const e of Object.values(data.stocks)){
  const forecasts=guide.allPeriods(ml,e);if(![126,252].every(h=>forecasts[h]))continue;
  for(const mode of ['center','sample']){
    const full=path.build(e,forecasts[252],252,mode,forecasts);
    for(const h of [126])assert.deepEqual(path.build(e,forecasts[h],h,mode,forecasts).points,full.points.slice(0,h+1));
    for(const h of [126,252]){if(mode==='center')assert.equal(full.points[h].base,forecasts[h].base);else if(full.sampled)assert.notEqual(full.points[h].base,forecasts[h].base);}
