...
// Exercise the new section even before the first real v7 artifact is published.
for(const v of Object.values(data.stocks))if(!v.experiments){
 const metric=v.allDates.environment;
 v.experiments={matchedRows:metric.n,dynamicInputRows:metric.n,evaluation:{noChange:{...metric},independentReturn:{...metric,returnDirectionAccuracy:.4,downRecall:.2,downPrecision:.5},secDynamics:{...metric,returnDirectionAccuracy:.45,downRecall:.3,downPrecision:.6}}};
 v.comparison.selectionEligible=false;
}
...
setImmediate(()=>{
 for(const cls of ['stocks','crypto'])for(const h of (cls==='stocks'?['0','1']:['0','1','2'])){
  ...
  if(cls==='stocks')assert(html.includes('방향과 목표가격 보완 실험')&&html.includes('하락 예측 적중'));
