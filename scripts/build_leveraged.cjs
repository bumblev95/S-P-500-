...
function build(market,now,root){
 const dir=path.join(root,'leveraged');fs.mkdirSync(dir,{recursive:true});
 ...
 if(!replay){const account=P.report(P.run(P.create('crypto',now,'leverage5x3x'),market,{mode:'replay',now}));
  const control=P.report(P.run(P.create('crypto',now),market,{mode:'replay',now}));
  replay={generatedAt:now,profileVersion:P.LEVERAGED.profileVersion,account,control:{equity:control.equity,return:control.return,maxDrawdown:control.maxDrawdown,trades:control.trades.length,startedAt:control.startedAt,marketAsOf:control.marketAsOf}};write('replay.json',replay);}
 if(replay.profileVersion!==P.LEVERAGED.profileVersion)throw Error('Preserve old leverage experiment');
 state=state||{createdAt:now,ledgerHash:null,account:P.create('crypto',now,'leverage5x3x')};
 ...
 if(events.length||state.ledgerHash===null){const record={profileVersion:P.LEVERAGED.profileVersion,recordedAt:now,previousHash:state.ledgerHash,events,equity:P.equity(next)};
 ...
 return {config:P.LEVERAGED,forward:{createdAt:state.createdAt,accounts:{crypto:P.report(next)}},replay:{generatedAt:replay.generatedAt,accounts:{crypto:replay.account}},control:replay.control};
}
