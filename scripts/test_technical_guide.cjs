...
const context={DecisionSupport:{load:async()=>{},applyStocks:x=>x,mountStock(){}},StabilityPanel:require('../assets/stability-panel.js'),TradeJournal:{mount(){}},ForecastPath:require('../assets/forecast-path.js'),TechnicalGuide:guide,LearnedGuide:require('../assets/learned-guide.js'),MarketContext:{render(){}},document:{querySelector:node,querySelectorAll:()=>[]},window:{scrollTo(){}},fetch:async url=>({ok:true,json:async()=>url.startsWith('forecasts')?data:url.startsWith('market')?market:url.startsWith('ml/latest')?learned:null}),console};
vm.runInNewContext(fs.readFileSync('index.html','utf8').match(/<script>([\s\S]*?)<\/script>/)[1],context);
setImmediate(()=>{const nvda=data.stocks.NVDA,ai=context.LearnedGuide.inspect(learned,nvda,126),shown=ai.researchForecast||nvda.predictions[126];assert(node('#app').innerHTML.includes('기술 지표'));assert(node('#app').innerHTML.includes(ai.researchForecast?'AI 연구 전망 · 검증 미통과':'AI 연구 자료 없음'));assert(node('#app').innerHTML.includes('$'+shown.base.toFixed(2)));assert(node('#app').innerHTML.includes('3년 가치 시나리오'));assert(!node('#app').innerHTML.includes('AI 전망 판단 보류'));assert(!/NaN|Infinity/.test(node('#app').innerHTML));console.log(`${count} plans, missing/stale guards, RSI/MACD flat series, page render passed`);});
