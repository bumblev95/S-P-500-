...
const context={StabilityPanel:require('../assets/stability-panel.js'),TradeJournal:{mount(){}},ForecastPath:require('../assets/forecast-path.js'),TechnicalGuide:guide,LearnedGuide:require('../assets/learned-guide.js'),MarketContext:{render(){}},document:{querySelector:node,querySelectorAll:()=>[]},window:{scrollTo(){}},fetch:async url=>({ok:true,json:async()=>url.startsWith('forecasts')?data:url.startsWith('market')?market:url.startsWith('ml/latest')?learned:null}),console};
vm.runInNewContext(fs.readFileSync('index.html','utf8').match(/<script>([\s\S]*?)<\/script>/)[1],context);
setImmediate(()=>{assert(node('#app').innerHTML.includes('기술 지표'));assert(node('#app').innerHTML.includes('AI 연구 전망 · 검증 미통과'));assert(node('#app').innerHTML.includes('$'+learned.stocks.NVDA.predictions[84].forecast.base.toFixed(2)));assert(!node('#app').innerHTML.includes('AI 전망 판단 보류'));assert(!/NaN|Infinity/.test(node('#app').innerHTML));console.log(`${count} plans, missing/stale guards, RSI/MACD flat series, page render passed`);});
