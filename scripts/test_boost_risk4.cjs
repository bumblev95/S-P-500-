...
async function testUI(){
 ...
 let html=await load();assert(html.includes('현재 모의운용'),html);assert(html.includes('$10,000.00'));assert(html.includes('$400.00'));assert(html.includes('현재 포지션 · 0 / 1개'));assert(!html.includes('424.64%'));assert(!html.includes('undefined')&&!html.includes('NaN'));
