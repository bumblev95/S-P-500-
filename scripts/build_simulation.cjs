...
function build(now=Date.now()){
 ...
 const boostStudy=require('./build_momentum_boost.cjs').build(market,now,folder);
 Object.assign(output,boostStudy.accounts);output.momentumBoostStudy=boostStudy.study;
 output.momentumBoost5xRisk4=require('./build_boost_risk4.cjs').build(market,now,folder);
 output.defaultCryptoProfile='momentumBoost5xRisk4';
 ...
 for(const asset of ['crypto','stocks']){
  ...
  for(const group of asset==='crypto'?[output,output.leveraged,output.wide,output.momentum,output.momentumTarget6,...Object.values(boostStudy.accounts)]:[output])for(const mode of ['forward','replay'])for(const trade of group[mode]?.accounts?.[asset]?.trades?.slice(-30)||[]){
 ...
 for(const group of [output.leveraged,output.wide,output.momentum,output.momentumTarget6,...Object.values(boostStudy.accounts)])for(const mode of ['forward','replay'])group[mode].accounts.crypto.events=group[mode].accounts.crypto.events.slice(-60);
