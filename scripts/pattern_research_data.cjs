...
function features(rows,higher,symbol,q){
 const r=rows.at(-1),a=E.indicators(rows.slice(-240),'15m'),hi=E.indicators(S.completed(higher,r.end),'1h');
 if(!a||!hi)return null;
...
function dataset(market,profile='leverage5x3x'){const out=[],hold=profile==='wideRecovery'?32:8;
 for(const [symbol,source] of Object.entries(market.crypto||{})){
  const rows=source.frames['15m'],higher=source.frames['1h'];
  P.funding(P.create('crypto',rows[0].t),{entryAt:rows[0].t,fundingThrough:rows[0].t,qty:0,side:'long',funding:0},rows[0].t,source,rows[0].close);
  for(let i=65;i<rows.length-hold;i++){
   const prefix=rows.slice(Math.max(0,i-260),i+1);let q=S.crypto(prefix,higher);if(profile==='wideRecovery')q=S.wide(q);if(!q.side)continue;
   const window=rows.slice(i,i+hold+1);if(window.some((r,k)=>k&&r.t-window[k-1].t!==900000))continue;
   ...
   const account=P.run(P.create('crypto',window[0].end,profile),markets,{mode:'replay',now:window.at(-1).end+1,startAt:window[0].t,provider});
...
if(require.main===module){const market=JSON.parse(fs.readFileSync(process.argv[2]||'simulation/market.json'));process.stdout.write(JSON.stringify(dataset(market,process.argv[3]||'leverage5x3x')));}
