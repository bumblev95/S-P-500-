...
function evaluate(filtered){
 ...
 const a=P.report(P.run(P.create('crypto',input.start,'leverage5x3x'),market,{mode:'replay',startAt:input.start-899999,now:input.end+1,provider}));
 return {equity:a.equity,return:a.return,maxDrawdown:a.maxDrawdown,trades:a.trades.length,winRate:a.winRate,fees:a.fees,funding:a.funding,positions:a.positions.length};
}
