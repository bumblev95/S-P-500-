...
def build(root=ROOT):
    ...
    for h,v in horizons.items():
        b=v['evaluation']['balanced'];e=v['evaluation']['enriched']
        lines.append(f"| {h}거래일 | {v['matchedIssuers']} / {v['matchedRows']} / {e['dates']} | {pct(b['mape'])} | {pct(e['mape'])} | {pct(b['directionAccuracy'])} | {pct(e['directionAccuracy'])} | {pct(v['alwaysUpDirectionAccuracy'])} | {pct(e.get('downRecall'))} |")
    lines += ['', '방향은 상승·중립(±2%)·하락의 세 범주입니다. 하락 포착률은 실제 하락한 표본 가운데 하락으로 예측한 비율입니다. 표본 수는 독립된 시장 상황의 수가 아니며, 이번 결과만으로 기본 모델을 자동 교체하지 않습니다.', '',
